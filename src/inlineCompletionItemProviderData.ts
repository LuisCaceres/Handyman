...
const dataSets: DataSet[] = [
    {
        title: 'Refactor `for... of` loop to iteration with `forEach` method.',
        // For example, it matches `items` in `const items`.
        // For example, it matches `elements` in `let elements`.
        regex: /(?<=(const|let)\s)\w+/,
        ...
        */
        generateCodeSnippet: function (lines) {
            const variable = lines.match(this.regex)?.[0] || '';
            const codeSnippet = ` Let \`${variable}\` be `;
            return codeSnippet;
        }
    },
    {
